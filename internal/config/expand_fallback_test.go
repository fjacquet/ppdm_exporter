...
func TestExpandFallback(t *testing.T) {
	unsetForTest(t, "PPDM_FALLBACK_TEST_UNSET")
...
func TestExpandSecretRejectsEmpty(t *testing.T) {
	t.Setenv("PPDM_EMPTY_SECRET_TEST", "")

	if _, err := interpolate("${PPDM_EMPTY_SECRET_TEST}"); err != nil {
		t.Fatalf("plain expansion must stay lenient on an exported-empty variable: %v", err)
	}
	if _, err := interpolateSecret("password", "${PPDM_EMPTY_SECRET_TEST}"); err == nil {
